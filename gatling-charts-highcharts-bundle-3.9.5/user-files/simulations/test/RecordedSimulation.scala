...
class RecordedSimulation extends Simulation {
  ...
  private val scn = scenario("RecordedSimulation")
    .exec(
      http("get_three_Cards_from_Stack")
    ...
    .pause(1)
    .exec(
      http("get_Cards_on_Field")
        .get("/playersAndPlayingfield/playingField/cardsOnPlayingField")
        .headers(headers_7)
    )
    .pause(1)
    .exec(
      http("get_current_Player")
        .get("/playersAndPlayingfield/players/getCurrentPlayer")
        .headers(headers_8)
    )
    .pause(1)
    .exec(
      http("set_current_Player_to_knocked")
        .get("/playersAndPlayingfield/players/setCurrentPlayerToKnocked")
        .headers(headers_9)
    )
    .pause(1)
    .exec(
      http("change_to_next_Player")
        .get("/playersAndPlayingfield/players/changeToNextPlayer")
        .headers(headers_10)
    )
    .pause(1)
    .exec(
    ...
    .pause(1)
    .exec(
      http("change_to_next_Player_again")
        .get("/playersAndPlayingfield/players/changeToNextPlayer")
        .headers(headers_12)
    )
    .pause(1)
    .exec(
      http("swap_one_Card")
        .post("/playersAndPlayingfield/swapOneCards")
        .headers(headers_13)
        .body(RawFileBody("test/recordedsimulation/0013_request.json"))
    )
    .pause(1)
    .exec(
      http("save_to_MongoDB")
        .get("/playersAndPlayingfield/save")
        .headers(headers_14)
    )
    .pause(1)
    .exec(
      http("load_from_MongoDB")
        .get("/playersAndPlayingfield/load")
        .headers(headers_15)
    )
